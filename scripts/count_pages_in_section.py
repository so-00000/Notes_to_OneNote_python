...
def _count_via_paging(client, section_id: str) -> int:
    ...
    url = (
        f"{GRAPH_ONENOTE_BASE_URL}/sections/{quote(section_id)}/pages"
        f"?$select=id,title&$top={PAGE_BATCH_SIZE}"
    )
...
def count_pages_in_section(*, section_name: str) -> int:
    ...
    try:
        ...
        print(
            ...
            f"section={section_name} section_id={section_id}"
        )
        ...
        for attempt in range(1, COUNT_RETRIES + 1):
            try:
                ...
                print(f"[INFO] strategy=odata_count count={result}")
                return result
            except Exception as exc:
                last_error = exc
                if attempt < COUNT_RETRIES:
                    print(
                        f"[WARN] odata_count retry {attempt}/{COUNT_RETRIES} "
                        f"section={section_name} error={exc}"
                    )
                    time.sleep(COUNT_RETRY_SLEEP_SECONDS)

        print(
            f"[WARN] fallback=paging section={section_name} "
            f"reason={last_error}"
        )
        result = _count_via_paging(client, section_id)
        print(f"[INFO] strategy=paging count={result}")
        return result
    finally:
        client.close()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Count pages in one target section in the configured notebook."
    )
    parser.add_argument(
        "--section-name",
        default="",
    ...
    count = count_pages_in_section(section_name=section_name)
